...
@pytest.mark.asyncio
async def test_increase_decrease_this_is_very_slow(
    ...
    for validator in validator_list.validators:
        assert (
            ...
            == increase_amount // 2 + stake_rent_exemption
        )
    ...
    for validator in validator_list.validators:
        assert (
            validator.transient_stake_lamports
            == increase_amount + stake_rent_exemption * 2
        )
        assert validator.active_stake_lamports == minimum_amount
    ...
    for validator in validator_list.validators:
        ...
        assert (
            ...
            == increase_amount + minimum_amount + stake_rent_exemption
        )

    # decrease from all
    futures = [
        decrease_validator_stake(
            async_client, payer, payer, stake_pool_address, validator, decrease_amount
        )
        for validator in validators
    ]
    await asyncio.gather(*futures)
    ...
    for validator in validator_list.validators:
        assert (
            validator.transient_stake_lamports == decrease_amount + stake_rent_exemption
        )
        assert (
            validator.active_stake_lamports
    ...
    expected_active_stake_lamports = (
        increase_amount - decrease_amount + minimum_amount + stake_rent_exemption
    )
