...
@pytest.mark.asyncio
async def test_rebalance_this_is_very_slow(
    ...
    # in Fogo MINIMUM_ACTIVE_STAKE is 100 SOL
    # however for the local testing environment,
    # the MINIMUM_ACTIVE_STAKE is 1 SOL as we are using the default stake program
    minimum_amount = LAMPORTS_PER_SOL + stake_rent_exemption
    ...
    await deposit_sol(
        async_client, payer, stake_pool_address, token_account, deposit_amount
    )

    # Test case 1: Increase everywhere
    await rebalance(ENDPOINT, stake_pool_address, payer, 0.0, 0.0)
    ...
    # some times it is 1 lamport off, not sure why
    assert resp.value.lamports - stake_rent_exemption + MINIMUM_RESERVE_LAMPORTS in [
        0,
        1,
    ]
    ...
    for validator in validator_list.validators:
        ...
        assert (
            ...
            == total_lamports // len(validators) - minimum_amount
        )
    ...
    await rebalance(
        ENDPOINT, stake_pool_address, payer, max_in_reserve / LAMPORTS_PER_SOL, 0.0
    )
    ...
    # loosen the assertion here as sometimes it is 1 lamport off, not sure why
    assert reserve_lamports - (stake_rent_exemption + MINIMUM_RESERVE_LAMPORTS) in [
        0,
        1,
    ]
    ...
    assert (
        reserve_lamports
        == stake_rent_exemption + max_in_reserve + MINIMUM_RESERVE_LAMPORTS
    )
